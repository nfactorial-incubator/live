...
export class WakaAPI {
    ...
    async getUserStats(username="yerlantemir", range="today") {
        try {
            const wakaStatsResp = await axios.get(`${WAKAAPI_ENDPOINT}/compat/wakatime/v1/users/${username}/summaries?range=${range}`, {
                headers
            });
            const data = wakaStatsResp.data.data?.[0]
            return {
                languages: data.languages?.map((languageStats) => languageStats.name),
